...
<% robotsMetaString %>
<title><% title %></title>
<link rel="stylesheet" type="text/css" href="https://cdnjs.cloudflare.com/ajax/libs/swagger-ui/5.17.2/swagger-ui.min.css" >
<% favIconString %>
<style>
html
{
...
interface SetupFunction {
  (swaggerDoc: any, opts?: any, options?: any, customCss?: any, customfavIcon?: any, swaggerUrl?: string, customSiteTitle?: string): (req: any, res: any) => void
}

const setup: SetupFunction = function (swaggerDoc, opts, options, customCss, customfavIcon, swaggerUrl, customSiteTitle) {
...
function swaggerInitFn(req: Request, res: Response, next: NextFunction) {
...
function swaggerAssetMiddleware(options?: NonNullable<unknown>) {
  const opts = options || {}
  opts.index = false
  return express.static(getAbsoluteSwaggerFsPath.absolutePath(), opts)
}

const serveFiles = function (swaggerDoc: any, opts: any) {
  opts = opts || {}
  const initOptions = {
    swaggerDoc: swaggerDoc || undefined,
...
const serveWithOptions = (options: any) => [swaggerInitFn, swaggerAssetMiddleware(options)]

let stringify = function (obj: any, prop: any) {
...
export { setup, serve, serveWithOptions, generateHTML, serveFiles }
