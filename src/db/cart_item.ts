...
export async function insertCartItemTable(cart_item: CartItem) {
...
export async function dropCartItemTable() {
  await pool.query('DROP TABLE "cart_item"')
}

export async function getCartItemByUserId(user_id: number) {
  const data = await pool.query<TableCartItem>(
`
SELECT *
...
export async function getCartItemByProductId(product_id: number) {
  try {
    const data = await pool.query<TableCartItem>(
`
SELECT *
 ...
 * @param type - `change` - заменяет, `update` - обновляет продукт в корзине
 */
export async function editCartItemByProductId(product_id: number, quantity: number, type: 'change' | 'update' = 'update') {
  const data = await pool.query<TableCartItem>(
`
UPDATE "cart_item"
SET ${
...
export async function deleteCartItemByProductId(product_id: number) {
  try {
    const data = await pool.query<TableCartItem>(
